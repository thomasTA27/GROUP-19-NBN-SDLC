...
Decisions made so every criterion can be tested. Where the project already has a convention, the assumption follows it and names the file. Each one should be confirmed or overridden by the product owner.

| ID | Assumption | Used by |
|---|---|---|
| A1 | A title can be at most 200 characters. Whether it can be empty is open question AC-2.1. *Source: docs/FIRESTORE-SCHEMA.md, the notes title limit.* | AC-2.2 |
| A2 | A title made only of spaces counts as empty. Leading and trailing spaces are removed when a title is saved. *Source: engineering decision; the project has no convention.* | AC-2.2 |
| A3 | A description can be at most 10,000 characters. *Source: docs/FIRESTORE-SCHEMA.md, the notes body limit.* | AC-2.3 |
| A4 | The description is plain text with no formatting. Line breaks the user types are kept and shown. *Source: engineering decision; the project has no convention.* | AC-2.3 |
| A5 | A date-only due date is a calendar date and shows as the same date on every device, whatever the device's timezone. A date-and-time due date is shown in the timezone of the device displaying it. Which of the two applies is open question AC-2.4. *Source: frontend/src/lib/utils.ts, where the app's date formatting uses the viewing device's timezone.* | AC-2.5 |
| A6 | There is no latest allowed due date: any date up to and including 31 December 9999, the last date with a four-digit year, is accepted. *Source: engineering decision; the project has no convention.* | AC-2.6 |
| A7 | A user can have more than one task with the same title. *Source: docs/FIRESTORE-SCHEMA.md, where no collection requires unique titles.* | AC-2.7 |
| A8 | Each list item shows the task's title, full description and due date, as well as its checkbox. There is no separate detail view. The edit view is the only other place a single task is shown. *Source: docs/GUIDE.md, where the notes list shows each note's title and body.* | AC-4.3, AC-7.1 |
| A9 | Tasks are listed by due date, soonest first. Tasks with the same due date are listed by creation time, oldest first. If the due date is optional (AC-2.1), tasks without one come after all tasks with one, oldest first. *Source: engineering decision; the notes list sets no order.* | AC-4.4 |
| A10 | Completed tasks appear in the same list as pending ones, in the same order. *Source: ticket. The checkbox in the list unticks completed tasks, so they have to be visible there. Hiding or grouping them would be new behaviour.* | AC-4.5 |
| A11 | Overdue tasks are not marked. *Source: ticket, which doesn't mention it. Marking would be a new feature.* | AC-4.6 |
| A12 | There is no limit on tasks per user, and the list shows all of them on one page, without paging. *Source: docs/GUIDE.md, where the notes list shows every note without paging.* | AC-4.1, AC-4.7 |
| A13 | The task list is on its own page in the signed-in area, reached from a "Tasks" link in the sidebar. *Source: docs/GUIDE.md step 4 and CLAUDE.md "Codebase Map": each feature gets its own dashboard page and a sidebar link, as notes does.* | AC-4.8 |
| A14 | List updates must appear within 3 seconds, measured on the test environment's connection with no network throttling. *Source: engineering decision; the project sets no performance targets.* | AC-4.11 |
| A15 | Changes made by the same user in another tab or on another device also appear without a reload, within the same time as A14. *Source: docs/ARCHITECTURE.md, where the app's lists are live subscriptions.* | AC-4.11 |
| A16 | Status can only be changed with the checkbox in the list. The edit view has no status control. *Source: ticket, which names the checkbox as the only status control.* | AC-5.3 |
...
| A18 | When the same task is edited in two places at once, the later save wins and no conflict warning is shown. *Source: engineering decision. Conflict detection would be a new feature.* | AC-5.5 |
| A19 | No completion time is recorded or shown. *Source: ticket, which doesn't mention it. It would be a new feature.* | AC-6.6 |
| A20 | Deleting a task needs an in-page confirmation step, because the user can't undo a delete (see A25). docs/DESIGN.md "Notifications" rules out the browser's built-in confirm dialog. Revisit this if the answer to AC-7.6 makes restoring tasks part of this ticket. *Source: engineering decision.* | AC-7.5 |
| A21 | Deleted task records are kept indefinitely. This ticket adds no way, automatic or manual, to erase them. *Source: docs/SECURITY.md "Soft-delete only"; the project has no purge mechanism.* | AC-7.7 |
| A22 | A successful delete shows a success message. A successful toggle doesn't; the checkbox change is the confirmation. *Source: docs/DESIGN.md "Notifications" for delete. For toggle, an engineering decision to avoid a message on every tick.* | AC-8.2 |
| A23 | The narrowest supported screen width is 320px and the widest is 1920px. 320px is the width used by the WCAG reflow criterion (1.4.10). docs/DESIGN.md caps page content at 1280px, so 1920px checks the layout on a wide screen. *Source: docs/DESIGN.md "Responsive breakpoints" and "Spacing"; the endpoints are an engineering decision.* | AC-8.3 |
| A24 | The accessibility standard is the rules in docs/DESIGN.md "Accessibility". No external standard is required. *Source: docs/DESIGN.md "Accessibility".* | AC-8.4 |
| A25 | Everything listed in section 9 is out of scope. Restoring deleted tasks stays out of scope unless the answer to AC-7.6 brings it in. *Source: ticket, which mentions none of them.* | §9, A20 |
...
- **AC-4.1** The task list contains every non-deleted task the user owns, on one page (see AC-4.7), and nothing else. [Ticket] [Assumes A12]
...
- **AC-4.7** There is no limit on the number of tasks a user can have, and the list shows all of them on one page, without paging. [Assumes A12]
...
- **AC-4.11** Within 3 seconds of the user's own create, edit, toggle or delete succeeding, the list shows the change, without a page reload or any other action by the user. The same applies when the change was made by the same user in another tab or on another device. [Project: docs/ARCHITECTURE.md, the app's lists update live] [Assumes A14, A15]
